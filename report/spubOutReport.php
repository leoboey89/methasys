<?php 
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST'){
        ...
        $spuboutResult = $db->getResultSet('spubout_mstr', ['spubout_patcode', 'spubout_patname', 'date_created'], 
            ['date_created >= "' . $fromDate . '"', 'date_created < "' . $newToDate->getMySQLFormat() . '"', 
            'spubout_active="Y"', "spubout_kk = '$kk'"]);
...
<html>
<head>
    <title>MethaSys-SPUB Out Report</title>
...
<div id="header">
    <?php require_once '../includes/titleHeader.php';?>
        <lable>MethaSys - SPUB Out Report Page</lable>
            ...
            <table width="100%" id="table">
                ...
                <?php
                    if ($_SERVER['REQUEST_METHOD'] == 'POST'){
                        ...
                        while ($row = $spuboutResult->fetch_assoc()) {
                            if ($count%2 == 0) {
                                echo '<tr class="even">';
                            } else {
                                echo '<tr>';
                            ...
                            echo '<td style="width:15%">' . $row['spubout_patcode'] . '</td>';
                            echo '<td style="width:45%;text-align:left;">' . $row['spubout_patname'] . '</td>';
