<?php    
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST'){
        ...
        /*Get lost_mstr count*/
        $lostResult = $db->getResultSet('lost_mstr', ['lost_patcode', 'lost_patname', 'date_created'], 
            ['date_created >= "' . $fromDate . '"', 'date_created < "' . $newToDate->getMySQLFormat() . '"', 
            'lost_active="Y"', "lost_kk = '$kk'"], ['lost_id'], ['lost_id']);
...
<html>
<head>
    <title>MethaSys-Lost Report</title>
...
<div id="header">
    <?php require_once '../includes/titleHeader.php';?>
        <lable>MethaSys - Lost Report Page</lable>
                ...
                <?php
                    if ($_SERVER['REQUEST_METHOD'] == 'POST'){
                        ...
                        while ($row = $lostResult->fetch_assoc()) {
                            ...
                            echo '<td style="width:15%">' . $row['lost_patcode'] . '</td>';
                            echo '<td style="width:45%;text-align:left;">' . $row['lost_patname'] . '</td>';
