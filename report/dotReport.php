<?php    
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST'){
        ...
        $dotResult = $db->getResultSet('methascan_hist', ['methascan_patcode', 'methascan_patname', 'methascan_dose', 'methascan_volume', 'date_created'], 
            ['methascan_date >= "' . $fromDate . '"', 'methascan_date < "' . $newToDate->getMySQLFormat() . '"', 
            'methascan_status="Y"', 'methascan_dot = "Y"', "methascan_kk = '$kk'"], ['methascan_id'], ['methascan_id']);
...
<html>
<head>
    <title>MethaSys-DOT Report</title>
...
<div id="header">
    <?php require_once '../includes/titleHeader.php';?>
        <lable>MethaSys - DOT Report Page</lable>
            ...
            <table width="100%" id="table">
                ...
                <?php
                    if ($_SERVER['REQUEST_METHOD'] == 'POST'){
                        ...
                        while ($row = $dotResult->fetch_assoc()) {
                            if ($count%2 == 0) {
                                echo '<tr class="even">';
                            } else {
                                echo '<tr>';
