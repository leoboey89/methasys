<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST'){
        ...
        $m1moutResult = $db->getResultSet('m1mout_mstr', ['m1mout_patcode', 'm1mout_patname', 'date_created'], 
            ['date_created >= "' . $fromDate . '"', 'date_created < "' . $newToDate->getMySQLFormat() . '"', 
            'm1mout_active="Y"', "m1mout_kk = '$kk'"]);
        $m1moutCount = $m1moutResult->num_rows;
...
<html>
<head>
    <title>MethaSys-M1M Out Report</title>
...
<div id="header">
    <?php require_once '../includes/titleHeader.php';?>
        <lable>MethaSys - M1M Out Report Page</lable>
            ...
            <table width="100%" id="table">
                ...
                <?php
                    if ($_SERVER['REQUEST_METHOD'] == 'POST'){
                        ...
                        while ($row = $m1moutResult->fetch_assoc()) {
                            if ($count%2 == 0) {
                                echo '<tr class="even">';
                            } else {
                                echo '<tr>';
                            ...
                            echo '<td style="width:15%">' . $row['m1mout_patcode'] . '</td>';
                            echo '<td style="width:45%;text-align:left;">' . $row['m1mout_patname'] . '</td>';
