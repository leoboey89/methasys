<?php    
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST'){
        ...
        $deathResult = $db->getResultSet('death_mstr', ['death_patcode', 'death_patname', 'date_created'], 
            ['date_created >= "' . $fromDate . '"', 'date_created < "' . $newToDate->getMySQLFormat() . '"', 
            'death_active="Y"', "death_kk = '$kk'"], ['death_id'], ['death_id']);
...
<html>
<head>
    <title>MethaSys-Death Report</title>
...
<div id="header">
    <?php require_once '../includes/titleHeader.php';?>
        <lable>MethaSys - Death Report Page</lable>
                ...
                <?php
                    if ($_SERVER['REQUEST_METHOD'] == 'POST'){
                        ...
                        while ($row = $deathResult->fetch_assoc()) {
                            ...
                            echo '<td style="width:15%">' . $row['death_patcode'] . '</td>';
                            echo '<td style="width:45%;text-align:left;">' . $row['death_patname'] . '</td>';
