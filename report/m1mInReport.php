<?php    
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST'){
        ...
        $m1minResult = $db->getResultSet('m1min_mstr', ['m1min_patcode', 'm1min_patname', 'date_created'], 
            ['date_created >= "' . $fromDate . '"', 'date_created < "' . $newToDate->getMySQLFormat() . '"', 
            'm1min_active="Y"', "m1min_kk = '$kk'"]);
        $m1minCount = $m1minResult->num_rows;
...
<html>
<head>
    <title>MethaSys-M1M In Report</title>
...
<div id="header">
    <?php require_once '../includes/titleHeader.php';?>
        <lable>MethaSys - M1M In Report Page</lable>
            ...
            <table width="100%" id="table">
                ...
                <?php
                    if ($_SERVER['REQUEST_METHOD'] == 'POST'){
                        ...
                        while ($row = $m1minResult->fetch_assoc()) {
                            if ($count%2 == 0) {
                                echo '<tr class="even">';
                            } else {
                                echo '<tr>';
                            ...
                            echo '<td style="width:15%">' . $row['m1min_patcode'] . '</td>';
                            echo '<td style="width:45%;text-align:left;">' . $row['m1min_patname'] . '</td>';
