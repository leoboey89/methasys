<?php    
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST'){
        ...
        $terminatedResult = $db->getResultSet('terminated_mstr', ['terminated_patcode', 'terminated_patname', 'date_created'], 
            ['date_created >= "' . $fromDate . '"', 'date_created < "' . $newToDate->getMySQLFormat() . '"', 
            'terminated_active="Y"', "terminated_kk = '$kk'"], ['terminated_id'], ['terminated_id']);
...
<html>
<head>
    <title>MethaSys-Terminated Report</title>
...
<div id="header">
    <?php require_once '../includes/titleHeader.php';?>
        <lable>MethaSys - Terminated Report Page</lable>
            ...
            <table width="100%" id="table">
                ...
                <?php
                    if ($_SERVER['REQUEST_METHOD'] == 'POST'){
                        ...
                        while ($row = $terminatedResult->fetch_assoc()) {
                            if ($count%2 == 0) {
                                echo '<tr class="even">';
                            } else {
                                echo '<tr>';
                            ...
                            echo '<td style="width:15%">' . $row['terminated_patcode'] . '</td>';
                            echo '<td style="width:45%;text-align:left;">' . $row['terminated_patname'] . '</td>';
