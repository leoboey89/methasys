<?php    
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST'){
        ...
        $transoutResult = $db->getResultSet('transout_mstr', ['transout_patcode', 'transout_patname', 'date_created'], 
            ['date_created >= "' . $fromDate . '"', 'date_created < "' . $newToDate->getMySQLFormat() . '"', 
            'transout_active="Y"', "transout_kk = '$kk'"], ['transout_id'], ['transout_id']);
...
<html>
<head>
    <title>MethaSys-Transfer Out Report</title>
...
<div id="header">
    <?php require_once '../includes/titleHeader.php';?>
        <lable>MethaSys - Transfer Out Report Page</lable>
            ...
            <table width="100%" id="table">
                ...
                <?php
                    if ($_SERVER['REQUEST_METHOD'] == 'POST'){
                        ...
                        while ($row = $transoutResult->fetch_assoc()) {
                            if ($count%2 == 0) {
                                echo '<tr class="even">';
                            } else {
                                echo '<tr>';
                            ...
                            echo '<td style="width:15%">' . $row['transout_patcode'] . '</td>';
                            echo '<td style="width:45%;text-align:left;">' . $row['transout_patname'] . '</td>';
