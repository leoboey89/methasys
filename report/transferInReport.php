<?php    
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST'){
        ...
        $transinResult = $db->getResultSet('transin_mstr', ['transin_patcode', 'transin_patname', 'date_created'], 
            ['date_created >= "' . $fromDate . '"', 'date_created < "' . $newToDate->getMySQLFormat() . '"', 
            'transin_active="Y"', "transin_kk = '$kk'"], ['transin_id'], ['transin_id']);
...
<html>
<head>
    <title>MethaSys-Transfer In Report</title>
...
<div id="header">
    <?php require_once '../includes/titleHeader.php';?>
        <lable>MethaSys - Transfer In Report Page</lable>
            ...
            <table width="100%" id="table">
                ...
                <?php
                    if ($_SERVER['REQUEST_METHOD'] == 'POST'){
                        ...
                        while ($row = $transinResult->fetch_assoc()) {
                            if ($count%2 == 0) {
                                echo '<tr class="even">';
                            } else {
                                echo '<tr>';
                            ...
                            echo '<td style="width:15%">' . $row['transin_patcode'] . '</td>';
                            echo '<td style="width:45%;text-align:left;">' . $row['transin_patname'] . '</td>';
