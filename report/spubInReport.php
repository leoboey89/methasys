<?php    
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST'){
        ...
        $spubinResult = $db->getResultSet('spubin_mstr', ['spubin_patcode', 'spubin_patname', 'date_created'], 
            ['date_created >= "' . $fromDate . '"', 'date_created < "' . $newToDate->getMySQLFormat() . '"', 
            'spubin_active="Y"', "spubin_kk = '$kk'"]);
...
<html>
<head>
    <title>MethaSys-SPUB In Report</title>
...
<div id="header">
    <?php require_once '../includes/titleHeader.php';?>
        <lable>MethaSys - SPUB In Report Page</lable>
            ...
            <table width="100%" id="table">
                ...
                <?php
                    if ($_SERVER['REQUEST_METHOD'] == 'POST'){
                        ...
                        while ($row = $spubinResult->fetch_assoc()) {
                            if ($count%2 == 0) {
                                echo '<tr class="even">';
                            } else {
                                echo '<tr>';
                            ...
                            echo '<td style="width:15%">' . $row['spubin_patcode'] . '</td>';
                            echo '<td style="width:45%;text-align:left;">' . $row['spubin_patname'] . '</td>';
