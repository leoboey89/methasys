<?php
    ...
    $result = $db->getResultSet(['patient_mstr','status_mstr'], ['patient_id', 'patient_code', 
        'patient_name', 'patient_methatype', 'patient_dose', 'patient_volume',
        'patient_age', 'patient_gender', 'patient_status', 'patient_ename', 
        'patient_econtact', 'patient_active', 'patient_mstr.user_created', 'patient_mstr.user_updated',
        'patient_mstr.date_created', 'patient_mstr.date_updated'], 
        ['patient_active = "Y"', 'patient_status in ("SUSPENDED","LOST","TERMINATED")',
        'patient_status = status_name', 'patient_kk = "' . $kk . '"', 
        'patient_m1min = "N"', 'patient_spubin = "N"'], ['patient_id'], ['status_id', 'patient_code']);
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (!empty($_POST['reactivate']) && !empty($_POST['patient-code'])) {
            ...
            if (!empty($missingInput)) {
                ...
                if (trim($_POST['reactivate']) == 'Reactivate') {
                    /*Update patient's info*/
                    ...
                    $updateSuccess = $db->updateData('patient_mstr', ["patient_name='$patientName'", 
                        ...
                        "patient_gender='$patientGender'", "patient_status='NORMAL'",
                        ...
                        "patient_state='$patientState'", "patient_lastreactivate='$date'",
                        "user_updated='$user'", "date_updated='$date'"], 
                        ["patient_code='$patientCode'", "patient_kk = '$kk'"]);

                    /*Insert patient's info into reactivate_mstr*/
                    $reactivateStatus = $db->insertData('reactivate_mstr',['reactivate_patcode', 
                        'reactivate_patname', 'reactivate_kk', 'user_created', 'date_created'],
                        [$patientCode, $patientName, $kk, $user, $date]);

                    /*Prompt update status for user*/
                    if ($updateSuccess && $reactivateStatus) {
                        echo '<script type="text/javascript">alert("' . $patientName . ', ' . $patientCode . ' Successful Reactivated!");</script>';
                        $_POST['patient-code'] = ''; 
                        $patientCode = '';
                        $patientName = '';
                        $patientMethatype = '';
        ...
        else if (!empty($_POST['patient-code']) && empty($_POST['reactivate'])) {
...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="../css/reactivateMainStyle.css">
            ...
            ajaxRequest.onreadystatechange = function() {
                if (ajaxRequest.readyState == 4 && ajaxRequest.status == 200) {
                    ...
                    if (jsonObj.patient_name.trim() != '') {
                        document.getElementById("reactivate").style.background = '#005C00';
                        document.getElementById("reactivate").disabled = false;
                        document.getElementById("patient-update-table").style.visibility = 'visible';
                    } else {
                        document.getElementById("reactivate").style.background = '#B80000';
...
<div id="header">
    <?php require_once '../includes/titleHeader.php';?>
        <lable>MethaSys - Reactivate Patient Page</lable>
        ...
        <div id="patient-maintenance">
            <div id="tabs">
                <form name="updateForm" method="post" autocomplete="off">
                    <label style='font-weight:bold;padding-left:5px;padding-right:114px;'>Reactivate :</label>
                    <select id="patient-code" name="patient-code" style="width:365px;" oninput="this.form.submit()">
                        <?php
                            while ($row = $result->fetch_assoc()) {
                                echo '<option value="' . $row['patient_code'] . '">' . $row['patient_code'] . ' >>> ' . $row['patient_name'] . '(' . $row['patient_status'] . ')</option>';
                            }
                        ?>
                    </select>
                    ...
                    <div id="patient-table-wrapper">
                        <table id="patient-update-table" style="visibility:
                            ...
                            <tr>
                                <th>Dose(mg)</th>
                                <td>
                                    <input type="number" min="5" max="100" step="5" id="patient-dose" name="patient-dose" onchange="autoCalVolume()">
                                </td>
                                <th>Address2</th>
                                <td>
                            ...
                            <tr>
                                <th>Volume(ml)</th>
                                <td>
                                    <input type="number" min="1" max="20" id="patient-volume" name="patient-volume" onchange="autoCalDose()">                                        
                                </td>
                                <th>Address3</th>
                                <td>
                        ...
                        <br>
                        <input type="submit" id="reactivate" name="reactivate" value="Reactivate" style="visibility:
